...
### B. 一鍵指令批次安裝
我們在 `scripts/` 目錄中準備好了一鍵安裝腳本 [`install_vscode_extensions.sh`](https://github.com/gemini960114/f1-docs/blob/main/04-ai-developer-tools/scripts/install_vscode_extensions.sh)：
...
```
*(執行 [`scripts/install_ai_cli.sh`](https://github.com/gemini960114/f1-docs/blob/main/04-ai-developer-tools/scripts/install_ai_cli.sh) 可自動完成檢查與 PATH 寫入)*
...
設定檔放置於：`~/.config/opencode/opencode.json`。以下為修正且通過驗證的標準配置方案（範本位於 [`templates/opencode.json`](https://github.com/gemini960114/f1-docs/blob/main/04-ai-developer-tools/templates/opencode.json)）：
...
本專案已在根目錄配置了開箱即用的標準規範：[`AGENTS.md`](./agents_governance)。其核心規範包括：
...
👉 **下一步**：進入 **[第 05 章：AI 輔助生醫管線 (FASTQ 質控登入節點實作)](./05_ai_assisted_bio_pipeline)**，指揮剛設定好的 AI 助手編寫資料分析管線！
