...
1. 在本地終端機編輯 `~/.ssh/config`（若檔案不存在會自動建立）：
   ...
   nano ~/.ssh/config
   ```
2. 貼上下列設定（本教學提供完整範本：[`config/ssh_config_example`](https://github.com/gemini960114/f1-docs/blob/main/01-f1-ssh-and-2fa/config/ssh_config_example)）：
   ```ini
   # 創進一號 X86 登入節點 (主要連線)
   Host f1
       HostName f1-ilgn01.nchc.org.tw
       User <您的主機帳號，如 c00cjz00>
       Port 22
...
👉 **下一步**：進入 **[第 02 章：網頁服務反向代理與動態埠解析](./02_web_service_reverse_proxy)**，學習如何突破超級電腦防火牆，在瀏覽器存取自己架設的網頁服務！
