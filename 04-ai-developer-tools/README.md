...
> [!NOTE]
> **🖥️ 瀏覽器操作與外網連線提醒**：
> 1. 請確認您已經透過瀏覽器進入第 03 章建立的 **Code-Server Web UI**。按下 **``Ctrl + ` ``** 展開整合式終端機，或點擊左側導覽列的 **Extensions 圖示 (`Ctrl + Shift + X`)**，本章所有套件安裝與 AI 設定都在這個瀏覽器視窗中直接完成！  
> 2. **外網依賴與節點架構說明**：
>    * **方案 A（登入節點 Code-Server，推薦）**：登入節點原生具備外網能力，可直接執行擴充套件安裝與 CLI 下載，無需任何額外設定。
>    * **方案 B（計算節點 Code-Server）**：計算節點預設無外網。若要安裝套件或下載模型，請確保登入節點已啟動 **[第 07 章的 Proxy 服務](../07-compute-node-proxy/)**（在登入節點執行 `bash ~/hpc-tutorial/07-compute-node-proxy/scripts/start.sh`），否則網路請求會連線逾時。
